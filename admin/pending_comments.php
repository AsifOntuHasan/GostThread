...
$comments = $conn->query("SELECT c.*, p.content as post_content FROM comments c LEFT JOIN posts p ON c.post_id = p.id WHERE c.status = 'pending' ORDER BY c.created_at DESC");
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <title>Pending Comments - GhostThread Admin</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .review-item { background: rgba(255, 255, 255, 0.03); border: 1px solid rgba(255, 255, 255, 0.08); border-radius: 12px; padding: 20px; }
        ...
        .review-content { color: rgba(255, 255, 255, 0.9); font-size: 15px; line-height: 1.6; margin-bottom: 15px; }
        ...
        .action-btn { padding: 10px 20px; border: none; border-radius: 8px; font-size: 13px; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 6px; }
        ...
        .toast { position: fixed; bottom: 30px; right: 30px; padding: 15px 25px; border-radius: 12px; color: white; font-weight: 500; display: none; }
...
<body>
    <div class="admin-layout">
        <aside class="sidebar">
            ...
            <nav class="nav-section">
                ...
                <a href="pending_comments.php" class="nav-item active"><i class="fas fa-comment-alt"></i><span>Pending Comments</span></a>
        ...
        <main class="main-content">
            <div class="header">
                <h1><i class="fas fa-comment-alt" style="color: #ffa502;"></i> Pending Comments</h1>
            ...
            <div class="content-section">
                <div class="section-header"><h2>Awaiting Review (<?php echo $comments->num_rows; ?>)</h2></div>
                
                <?php if ($comments->num_rows > 0): ?>
                <div class="review-list">
                    <?php while ($comment = $comments->fetch_assoc()): ?>
                        <?php $flagged = $comment['flagged_words'] ? json_decode($comment['flagged_words']) : []; ?>
                        <div class="review-item" id="comment-<?php echo $comment['id']; ?>">
                            <div class="review-header">
                                <div class="review-meta">
                                    <span class="review-type">Comment #<?php echo $comment['id']; ?></span>
                                    <span class="review-time"><?php echo date('M j, Y g:i A', strtotime($comment['created_at'])); ?></span>
                                </div>
                            </div>
                            <div class="post-preview"><strong>On Post:</strong> <?php echo htmlspecialchars(substr($comment['post_content'], 0, 100)); ?>...</div>
                            <p class="review-content"><?php echo htmlspecialchars($comment['content']); ?></p>
                ...
                <?php else: ?>
                <div class="empty-state"><i class="fas fa-check-circle"></i><p>No pending comments!</p></div>
                <?php endif; ?>
            </div>
        </main>
    </div>
    ...
    <script>
        ...
        function moderateComment(id, action) { fetch('api/moderate.php', { method: 'POST', headers: {'Content-Type': 'application/json'}, body: JSON.stringify({id, type: 'comment', action}) }).then(r=>r.json()).then(d=>{ if(d.success){ showToast(d.message,'success'); const e=document.getElementById(`comment-${id}`); if(e){e.style.opacity='0';setTimeout(()=>e.remove(),300);} } else showToast(d.message,'error'); }); }
